...
class IpStorageTests : public ::testing::Test
{
    // protected for GTest
  protected:
    IpStorageTests() : _printer(_actOutput) {}
...
TEST_F(IpStorageTests, PrintFilteredByFirstBytes)
{
    ...
    std::copy_if(_inputIps.begin(),
                 _inputIps.end(),
                 std::back_inserter(expIps),
                 [&firstByte, &secondByte](const hw1::IpV4& ip)
                 {
                     return ip.byte(0) == firstByte && ip.byte(1) == secondByte;
                 });
...
TEST_F(IpStorageTests, PrintIpsContainsByte)
{
    ...
    std::copy_if(_inputIps.begin(),
                 ...
                 {
                     return ip.byte(0) == byteValue || ip.byte(1) == byteValue || ip.byte(2) == byteValue ||
                            ip.byte(3) == byteValue;
                 });

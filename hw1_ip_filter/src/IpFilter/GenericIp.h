...
class GenericIp
{
  ...
  public:
    ...
    template <typename... Bytes,
              typename = hw_libs::sfinae::AreTypesSame<Byte, Bytes...>,
              typename = std::enable_if_t<sizeof...(Bytes) == _bytesNumber>>
    explicit GenericIp(Bytes... bytes) noexcept : _data{bytes...}
    {
    }

    GenericIp(const GenericIp& other) noexcept : _data(other._data) {}

    GenericIp(GenericIp&& other) noexcept : _data(std::move(other._data)) {}
    ...
    bool operator<(const GenericIp& other) const { return _data < other._data; }

    bool operator>(const GenericIp& other) const { return _data > other._data; }

    bool operator==(const GenericIp& other) const { return _data == other._data; }

    static constexpr size_t bytesNumber() { return _bytesNumber; }
